...
/**
 * The MySQL backend for communicating with the database, using the legacy mysql interface.
...
class b8_storage_mysql extends b8_storage_base
{
        ...
        if ($this->config['connection'] !== null) {
            ...
            if (is_resource($this->config['connection']) === true) {
                ...
                if ($resource_type != 'mysql link' and $resource_type != 'mysql link persistent') {
                    throw new Exception(
                        "b8_storage_mysql: The resource passed via the \"connection\" " .
                        "paramter is no MySQL resource but \"$resource_type\""
                    );
                }
            } else {
                throw new Exception(
                    'b8_storage_mysql: The resource passed via the "connection" ' .
                    'paramter is no resource.'
                );
            }
            ...
            if ($this->_connection === false
                or mysql_select_db($this->config['database'], $this->_connection) === false) {
    ...
    protected function _put($token, $count)
    {
        array_push($this->_puts,
            "('"
            . mysql_real_escape_string($token, $this->_connection)
            . "', '"
            . mysql_real_escape_string($count['count_ham'], $this->_connection)
            . "', '"
            . mysql_real_escape_string($count['count_spam'], $this->_connection)
            . "')"
        );
    }
    ...
    {
        array_push($this->_deletes, mysql_real_escape_string($token, $this->_connection));
    }
